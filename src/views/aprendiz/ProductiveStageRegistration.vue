<template>
    <div class="q-pa-md">
        ...
        <div class="container">
            ...
            <div class="row items-center q-mb-md q-gutter-y-sm">
                <div class="col-12 col-md-4">
                    <q-input v-model="searchTerm" dense outlined placeholder="Buscar registro...">
                ...
                <div class="col-12 col-md-4">
                    <q-select outlined dense v-model="statusFilter" :options="registrationStatusOptions" emit-value
                        map-options label="Estado" />
                </div>
                <div class="col-12 col-md-4 text-right">
                    <BotonIngresar label="+ Nuevo Registro EP" @click="handleOpenCreateDialog"
                        style="height: 36px; padding: 8px 16px;" />
                </div>
            </div>

            <!-- Tabla de mis registros -->
            <maintable :datos="filteredItems" :columnas="columns" row-key="_id"
                no-data-label="No tienes registros de EP. ¡Crea tu primer registro!"
                :rows-per-page-options="[10, 20, 50]" :initial-rows-per-page="10">
        ...
        <!-- Modal de Detalles -->
        <modalComponent ref="detailsModalRef" width="800px" max-width="98vw">
            <template #header>
        ...
        <modalComponent ref="editModalRef" width="900px" max-width="98vw">
            <template #header>
                    ...
                    <div class="row q-col-gutter-md">
                        ...
                        <div class="col-12 col-md-6">
                            <q-select v-model="itemBeingEdited.modality_id" :options="modalityOptions"
                                ...
                                outlined dense :rules="[validation.requiredRule]"
                        ...
                        <!-- Seleccionar Empresa -->
                        <div class="col-12 col-md-6">
                            <q-select v-model="itemBeingEdited.company_id" :options="companyOptions" option-value="_id"
                                option-label="name" emit-value map-options label="Empresa *" outlined dense
                                :rules="[validation.requiredRule]" hint="Empresa donde realizarás la EP" use-input
                                input-debounce="300" @filter="filterCompanies">
                                <template v-slot:no-option>
                                    <q-item>
                                        <q-item-section class="text-grey">
                        ...
                        <div class="col-12 col-md-6">
                            <q-input v-model="itemBeingEdited.scheduled_start_date" label="Fecha de Inicio Programada *"
                                outlined dense type="date" :rules="[validation.requiredRule]"
                            ...
                            <div class="q-pa-md"
                                ...
                                <div class="text-center">
                                    <q-file v-model="uploadedFile" label="Seleccionar archivo PDF *"
                                        ...
                                        :rules="[validation.requiredRule]" @update:model-value="handleFileSelect"
                                        clearable>
                                        <template v-slot:prepend>
                        ...
                        <div class="col-12 q-mt-md">
                            <div class="row justify-end q-gutter-sm">
                                ...
                                />
                                <BotonEnviar 
                                    v-if="isEditing && canSend(itemBeingEdited)"
                                    label="Enviar a Validación" 
                                    @click="handleSendFromModal"
                                    :loading="isSaving" 
                                />
                            </div>
                        </div>
...
// Usar las opciones de estado de registro
...
const { filteredItems, searchTerm, statusFilter } = useTableFiltering(registros, ['registration_number', 'modality', 'company'], 'registration_status')
...
const requiredDocuments = computed(() => {
    ...
    // Documentos por defecto si no hay coincidencia
    return [
...
/**
 * Observa cambios en la modalidad seleccionada para cargar automáticamente las horas estándar
 */
...
async function fetchRegistrations() {
    try {
        ...
        // Normalizar posibles formas de la respuesta (camelCase vs snake_case)
...
function canSend(record) {
    // Solo se puede enviar si está pendiente o rechazado (aún no aprobado)
    return record.registration_status === REGISTRATION_STATUS.PENDING ||
        record.registration_status === REGISTRATION_STATUS.REJECTED
}
...
async function handleSubmitAndSend() {
    ...
    try {
        ...
        // Si se creó exitosamente, no necesitamos enviar otra petición
        // porque el backend ya lo crea con status PENDING (3)
...
async function createRegistration() {
    try {
        ...
        // Generar número de registro único (formato: REG-YYYYMMDD-TIMESTAMP)
        const now = new Date()
    ...
    // Usar claves en camelCase porque el backend las espera así
...
async function updateRegistration() {
    try {
        ...
        if (uploadedFile.value) {
            const formData = new FormData()
            // En actualización también usar camelCase
            formData.append('modalityId', itemBeingEdited.value.modality_id)
            formData.append('companyId', itemBeingEdited.value.company_id)
            formData.append('scheduledStartDate', itemBeingEdited.value.scheduled_start_date)
            formData.append('totalRequestedHours', itemBeingEdited.value.total_requested_hours)
...
async function loadModalities() {
    try {
        ...
        // Filtrar solo las modalidades activas (status = 0) usando while
    ...
    } catch (error) {        notifications.error('Error al cargar las modalidades. Por favor, recarga la página.')
    }
}
...
async function loadCompanies() {
    try {
        ...
        // Filtrar solo las empresas activas (status = 0) usando while
        allCompanies.value = []
        let i = 0
        while (i < data.length) {
            if (data[i].status === 0) {
    ...
    } catch (error) {        notifications.error('Error al cargar las empresas. Por favor, recarga la página.')
    }
}
...
function filterCompanies(val, update) {
    update(() => {
        if (val === '') {
            // Copiar allCompanies usando while
            companyOptions.value = []
            let i = 0
            while (i < allCompanies.value.length) {
                companyOptions.value.push(allCompanies.value[i])
            ...
            while (i < allCompanies.value.length) {
                ...
                // Verificar si companyName contiene needle usando while
                let found = false
