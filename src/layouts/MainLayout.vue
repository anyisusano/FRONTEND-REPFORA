      ...
      <div class="header-container">
        ...
        <div class="header-action-btn">
          <q-btn fab color="white" size="lg" @click="showMorph = !showMorph" unelevated>
            <span style="font-weight:bold; color:var(--color-primary);">yo</span>
          </q-btn>
        </div>
      </div>
    </q-header>

    <!-- Drawer MEJORADO: se abre/cierra con el botón hamburguesa -->
    <q-drawer 
      ...
      <div class="drawer-header-enhanced">
        <div class="drawer-header-content">
          ...
          <div class="drawer-header-text">
            <div class="drawer-text-line-1">{{ firstName && lastName ? `${firstName} ${lastName}` : 'PRUEBAS ETAPAS' }}</div>
            <div class="drawer-text-line-2">ETAPA PRODUCTIVA</div>
            <div class="drawer-text-line-3">{{ roleLabel === 'ETAPA PRODUCTIVA VIRTUAL' || roleLabel === 'ETAPA PRODUCTIVA PRESENCIAL' ? (roleLabel.includes('VIRTUAL') ? 'VIRTUAL' : 'PRESENCIAL') : 'VIRTUAL' }}</div>
          </div>
        </div>
      </div>
      <div class="drawer-separator-enhanced"></div>
      <q-scroll-area class="drawer-scroll-area">
        <!-- Lista mejorada de navegación por rol -->
...
// Sin usar filter - el backend debe contar las notificaciones no leídas
const unreadCount = computed(() => {
  let count = 0
  let index = 0
  while (index < notifications.value.length) {
    if (notifications.value[index] && !notifications.value[index].read) {
      count++
    }
    index++
  }
...
function deleteNotification(id) {
  // Sin usar findIndex - buscar con while loop
  let idx = -1
  let index = 0
  while (index < notifications.value.length && idx === -1) {
    if (notifications.value[index] && notifications.value[index].id === id) {
      idx = index
    }
    index++
  }
